...
local function createDialog(selType, selMode)
    ...
    table.insert(args, "xdotool")
    table.insert(args, "getwindowfocus")
    local focus = utils.subprocess({ args = args })
    local kargs, zargs = {}, {}, {}
    args = {}
    ...
    if (flatpak == true) then
        table.insert(kargs, "flatpak-spawn")
        table.insert(kargs, "--host")
        table.insert(zargs, "flatpak-spawn")
        table.insert(zargs, "--host")
    end
